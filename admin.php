...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_event'])) {
    ...
    // Fetch LocationID based on city
...
// Fetch participants based on location
$participants = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_participants'])) {
    $eventID = $_POST['eventID']; // Correct name for event ID
    $donors = $_POST['donors'] ?? []; // Corrected to match form input names
    $volunteers = $_POST['volunteers'] ?? [];
    $supervisors = $_POST['supervisor'] ?? []; // Corrected name for supervisor array
    ...
    //     $deliveryTime = $_POST['delivery_time_' . $supervisorID];
    ...
    //     if (!$stmt->execute()) {
    //         echo "<p>Error adding supervisor $supervisorID: " . $stmt->error . "</p>";
    //     }
    // }
    foreach ($supervisors as $userID) {
        // Retrieve SupervisorID using UserID
        ...
        if ($result->num_rows > 0) {
            ...
            // Supervisor exists, proceed with insertion
